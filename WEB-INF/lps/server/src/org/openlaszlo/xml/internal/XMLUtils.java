...
abstract public class XMLUtils {
    ...
    private static StringBuffer getXPathToInternal(Element elt) {
        ...
        int i = parent.getChildren().indexOf(elt);
        if (i == -1) {
            throw new RuntimeException("Element " + elt + " not in parent " + parent + " .getChildren()?");
        }
        StringBuffer sb = getXPathToInternal(parent);
        sb.append("/*[");
        // DOM is 1-based
        sb.append(i + 1);
        sb.append("]");
    ...
    /**
     * Returns an xpath for the element
     *
     * @param elt the element
     * @return the xpath
     */
    public static String getXPathTo(Element elt) {
        return getXPathToInternal(elt).toString();
    }
}
