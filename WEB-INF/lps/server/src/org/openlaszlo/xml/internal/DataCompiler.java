...
public class DataCompiler extends DataCommon {
    ...
    public static InputStream compile(String x, String headers,
        ...
        return compile(x, headers, flashVersion, addwrapper, trimWhitespace, false, false);
    }
    ...
    public static InputStream compile(String x, String headers,
        ...
        return compile(x,headers,flashVersion,addwrapper,trimWhitespace,compress,false);
    }
    ...
    public static InputStream compile(String x, String headers,
        ...
        try {
            ...
            return getSWF(xppbody, xppheaders, x.length(), flashVersion, addwrapper, trimWhitespace, compress, nsprefix);
    ...
    public static InputStream compile(String x, int flashVersion, boolean nsprefix) throws IOException, DataCompilerException {
        try {
            // XML Parser for the data body
            XmlPullParser xppbody = getXPPFactory().newPullParser();
            xppbody.setInput( new StringReader(x) );
            ...
            return getSWF(xppbody, null, x.length(), flashVersion, false, trimWhitespace, false, nsprefix);
        } catch (XmlPullParserException ex) {
            throw new DataCompilerException("Parsing XML: " + ex.getMessage());
        }
    }
    ...
    */
    ...
    private static final DataContext writeXMLData(XmlPullParser xpp,
        ...
        DCOptions options = (DCOptions) ioptions.clone();

        int flashVersion       = options.flashVersion;
        boolean splitframes    = options.splitframes;
        boolean addwrapper     = options.addwrapper;
        boolean trimWhitespace = options.trimWhitespace;
        boolean localdata      = options.localdata;
        boolean nsprefix       = options.nsprefix;
        ...
        while (eventType != xpp.END_DOCUMENT) {
            if(eventType == xpp.START_DOCUMENT) {
                ...
                String eltprefix = xpp.getPrefix();
                if (nsprefix && eltprefix != null) {
                    eltname = eltprefix + ":" + eltname;
                } 
                ...
                if (xpp.isWhitespace()) {
                    // If the text is all whitespace, then don't create a node.
                } else {
    ...
    **/
    ...
    public static Program makeProgram(Element data, int flashVersion) throws CompilationError {
        ...
        options.flashVersion = flashVersion;
        options.trimWhitespace = false;
        options.localdata = false;

        return makeProgram(data, options);
    }
    ...
    */
    public static Program makeProgram(Element data, int flashVersion, boolean trimWhitespace, boolean localdata, boolean nsprefix) throws CompilationError {

        DCOptions options = new DCOptions();
        options.flashVersion = flashVersion;
        options.trimWhitespace = trimWhitespace;
        options.localdata = localdata;
        options.nsprefix = nsprefix;
        return makeProgram(data, options);
    }
    ...
    /**
       Called for compile-time data, don't do frame splitting, and don't add any resultset wrapper
    */
    public static Program makeProgram(Element data, DCOptions ioptions) throws CompilationError {

        DCOptions options = (DCOptions) ioptions.clone();
        options.splitframes = false;
        ...
        try {
            ...
            Vector progs = makeProgram(xpp, null, x.length(), options);
    ...
    /** Bag to hold data compiler options */
    static class DCOptions implements Cloneable {
        /** swf target runtime */
         int flashVersion = 6;

        /** Split code for building data into multiple frames, to keep player from
         * getting too sluggish or "player responding slowly" error */
         boolean splitframes = true;

        /** add metadata headers info as XML wrapper */
         boolean addwrapper = false;

        /** trim  whitespace in text content */
         boolean trimWhitespace = false;

        /** data inlined at compile time */
         boolean localdata = false;

        /** add namespace prefixes to tags and attribute names */
        ...
        public String toString() {
            return ("flashVersion="+flashVersion+", "+
    ...
    public static Vector makeProgram(XmlPullParser xpp, XmlPullParser xpheaders,
      ...
      throws IOException, XmlPullParserException {
        ...
        dc = writeXMLData(xpp, programs, dc, stackdepth, options);
        ...
        if (addwrapper) {
            ...
            options.splitframes = false;
            dc = writeXMLData(xpheaders, programs, dc, stackdepth, options);
            bodyp = (Program) programs.lastElement();
            body = bodyp.body();
        ...
        if (!localdata) {
            // Call to _parent.loader.returnData(_parent, data) 
            bodyp.push("_parent");
            bodyp.getVar();
            bodyp.push(2); 
            bodyp.push("_parent"); 
    ...
    private static FlashFile makeSWF(XmlPullParser xpp, XmlPullParser xpheaders, int xmlsize,
      ...
      throws IOException, XmlPullParserException {
        return makeSWF(xpp, xpheaders, xmlsize, flashVersion, addwrapper, trimWhitespace, false);
    }
     ...
     */
    private static FlashFile makeSWF(XmlPullParser xpp, XmlPullParser xpheaders, int xmlsize,
                                     int flashVersion, boolean addwrapper, boolean trimWhitespace, boolean nsprefix)
      throws IOException, XmlPullParserException {
        DCOptions options = new DCOptions();
        options.flashVersion = flashVersion;
        options.trimWhitespace = trimWhitespace;
        options.localdata = false;
        options.splitframes = true;
        options.addwrapper = addwrapper;
        options.nsprefix = nsprefix;
        ...
        Vector progs = makeProgram(xpp, xpheaders, xmlsize, options);
    ...
    public static InputStream getSWF(XmlPullParser xpp, XmlPullParser xpheaders, int xmlsize,
      ...
      throws IOException, XmlPullParserException {
        return getSWF(xpp, xpheaders, xmlsize, flashVersion, addwrapper, true, false, false);
    }
    ...
    public static InputStream getSWF(XmlPullParser xpp, XmlPullParser xpheaders, int xmlsize,
                                     int flashVersion, boolean addwrapper, boolean trimWhitespace,
                                     boolean compress, boolean nsprefix)
      throws IOException, XmlPullParserException {
        // Get inputstream and write to outputstream
        InputStream input;
        int i = 0;
        try {
            FlashFile file = makeSWF(xpp, xpheaders, xmlsize, flashVersion, addwrapper, trimWhitespace, nsprefix);
            if (flashVersion > 5) {
                if (compress) {
