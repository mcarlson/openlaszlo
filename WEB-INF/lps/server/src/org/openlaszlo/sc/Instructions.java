...
/* (non-Javadoc)
 * @i18n.test
 * @org-mes="offset out of range in " + p[0]
 */
            org.openlaszlo.i18n.LaszloMessages.getMessage(
                Instructions.class.getName(),"051018-366", new Object[] {this})
);
          }
...
/* (non-Javadoc)
 * @i18n.test
 * @org-mes="too many arguments in " + p[0]
 */
            org.openlaszlo.i18n.LaszloMessages.getMessage(
                Instructions.class.getName(),"051018-386", new Object[] {this})
);
        }
...
/* (non-Javadoc)
 * @i18n.test
 * @org-mes="invalid register number"
 */
            org.openlaszlo.i18n.LaszloMessages.getMessage(
                Instructions.class.getName(),"051018-408")
);
        }
...
/* (non-Javadoc)
 * @i18n.test
 * @org-mes="unimplemented: code generation for " + p[0]
 */
            org.openlaszlo.i18n.LaszloMessages.getMessage(
                Instructions.class.getName(),"051018-419", new Object[] {this.toString()})
);
      }
    }
    ...
    public int argsBytes() {
      ...
      if (op == Actions.CONSTANTS) {
        ...
        try {
          String encoding = "UTF-8";
          for (Iterator i = this.args.iterator(); i.hasNext(); ) {
            b += ((String)i.next()).getBytes(encoding).length + 1;
          }
        } catch (UnsupportedEncodingException e) {
          assert false : "this can't happen";
        }
   ...
   */
  public abstract static class TargetInstruction extends ConcreteInstruction {
    ...
    public abstract TargetInstruction makeTargetInstruction(List args);
  ...
  public static class WITHInstruction extends TargetInstruction {

    protected WITHInstruction(Action op) {
      this(op, null);
    }

    protected WITHInstruction(Action op, List args) {
      this(op, args, (short)0);
    }

    protected WITHInstruction(Action op, List args, short targetOffset) {
      super(op, args);
      assert op == Actions.WITH;
      this.targetOffset = targetOffset;
    }

    public TargetInstruction makeTargetInstruction(List args) {
      return new WITHInstruction(this.op, args);
    }
  }


  public static class BranchInstruction extends TargetInstruction {
    ...
    protected BranchInstruction(Action op) {
      this(op, null);
    }

    protected BranchInstruction(Action op, List args) {
      this(op, args, (short)0);
    }

    protected BranchInstruction(Action op, List args, short targetOffset) {
      super(op, args);
      assert OPCODES.contains(op);
      this.targetOffset = targetOffset;
    }

    public TargetInstruction makeTargetInstruction(List args) {
      return new BranchInstruction(this.op, args);
    }
  }
  ...
  public static class DefineFunctionInstruction extends TargetInstruction {
    ...
    protected DefineFunctionInstruction(Action op, List args, short targetOffset) {
      super(op, args);
      assert op == Actions.DefineFunction || op == Actions.DefineFunction2 : "Invalid op " + op.toString();
      this.targetOffset = targetOffset;
    }
...
/* (non-Javadoc)
 * @i18n.test
 * @org-mes="too many arguments"
 */
            org.openlaszlo.i18n.LaszloMessages.getMessage(
                Instructions.class.getName(),"051018-585")
);
        }
        bytes.putShort((short)nargs);
        for (Iterator i = fnargs.iterator(); i.hasNext(); ) {
          bytes.put(((String)i.next()).getBytes("UTF-8"));
          bytes.put((byte)0);
        }
        bytes.putShort(this.targetOffset);
      } catch (UnsupportedEncodingException e) {
      ...
      try {
        ...
        for (Iterator i = fnargs.iterator(); i.hasNext(); ) {
          ...
          if (arg instanceof Register) {
            ...
            bytes.put((byte)0);
            // string arg
            bytes.put(((String)arg).getBytes("UTF-8"));
            bytes.put((byte)0);
          } else {
