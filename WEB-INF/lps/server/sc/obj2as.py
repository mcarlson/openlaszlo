...
def URLdecodeAttributes(attrs):
    ...
    for idx in range(attrs.length):
      ...
      nattrs.addAttribute(attrs.getURI(idx),
                          ...
                          attrs.getQName(idx),"CDATA",LzHttpUtils.urldecode(aval, "UTF-8"))
    return nattrs
...
class Compiler(DefaultHandler):
    ...
    def visitObject(self, attrs, mode):
        ...
        # so we don't try to optimize INIT vs. EDIT
            ...
            self.emit(InitArray)
            mode = EDITING_ARRAY
        elif constructor == 'MovieClip':
            # Subclass of movieclip, not to be confused with a library movie
            self.push(0)
            self.push(constructor)
            self.emit(NEW)
            mode = EDITING_OBJECT
...
def optimize(fname, props, includeObjectHierarchy=True):
    ...
    # Do Flash 6 compression on file
