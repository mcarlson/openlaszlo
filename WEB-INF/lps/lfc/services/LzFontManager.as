...
//=============================================================================
// DEFINE OBJECT: LzFontManager
//
//=============================================================================
LzFontManager = new Object;

// currently doesn't support sending variables...
...
LzFontManager.addFont = function (  fontname , n , b , i , bi ){
    ...
    if ( n ){
        new _root.LzFont( ff , n, "plain");
    }
    if ( b ) { 
        new _root.LzFont( ff , b , "bold");
    }
    if ( i ){
        new _root.LzFont( ff , i, "italic" );
    }
    if ( bi ){
        new _root.LzFont( ff , bi , "bolditalic");
    }
...
//-----------------------------------------------------------------------------
// Get the named <b><i>LzFont</i></b> of the given style.
// 
// @param String fontname: The name of the font.
// @param String style: The style of the font.  One of "plain", "bold", "italic"
// or "bolditalic".  If style is null, "plain" is assumed.
// @return any: An <b><i>LzFont</i></b> object, or undefined if not found
//
//-----------------------------------------------------------------------------
LzFontManager.getFont = function ( fontname , style ){
   ...
   //@field dictionary fonts: A hash holding named font objects.  Each font object
   // contains slots for the styles that are available 
   // (plain, bold, italic, and/or bolditalic)
}

//-----------------------------------------------------------------------------
// Apply/add the requested style to the given <b><i>LzFont</i></b> and
// return the new <b><i>LzFont</i></b>.
//
// @param f: An <b><i>LzFont</i></b>.
// @param s: the style string (One of "plain", "bold", "italic", or "bolditalic").
// @return LzFont:  The new <b><i>LzFont</i></b>.
// @keywords private
//-----------------------------------------------------------------------------
    ...
    if ($debug) {
        for (var i in map)
            if (name == map[i] && !this.__debugWarnedFonts[name]) {
                ...
                _root.Debug.write("Undefined font '" + name + "'.  Use '" + i + "' instead.");
            }
    }
...
// none of the names can be rendered.  str is a comma-separate list
// of font names.
//-----------------------------------------------------------------------------
LzFontManager.__findMatchingFont = function (str) {
    ...
    if ($debug) {
        if (!LzFontManager.__fontExists(name) && !this.__debugWarnedFonts[name]) {
            this.__debugWarnedFonts[name] = true;
            _root.Debug.write("Undefined font '" + name + "'.");
        }
    }
