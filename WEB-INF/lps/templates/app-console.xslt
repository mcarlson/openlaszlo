...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="canvas">
    ...
    <xsl:when test="@runtime = 'dhtml'">
        <script type="text/javascript">
        lz.embed.dhtml({url: '<xsl:value-of select="$lps"/>/lps/admin/dev-console.lzx.js?lzappuid=<xsl:value-of select="$appuid"/>&amp;lzt=dhtml&amp;appinfo=<xsl:value-of select="$appinfo"/>', bgcolor: '#858599', width: '100%', height: '<xsl:value-of select="$consoleheight"/>', appenddivid: 'console', cancelkeyboardcontrol: true, resourceroot: '<xsl:value-of select="/canvas/request/@lps"/>', history: false});
        </script>
    </xsl:when>
    </xsl:choose>
    </xsl:if>
