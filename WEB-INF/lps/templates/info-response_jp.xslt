...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:param name="title" select="concat('コンパイル統計値: ', canvas/@title)"/>
    ...
    <html>
      ...
      <body>
        ...
        <xsl:if test="$showpercent">
          <p>
	　　以下のテーブルは<em>非圧縮</em>アプリケーションサイズについての一覧を表示しています。
             <em>圧縮済</em>アプリケーションではクラス定義とインスタンスサイズのパーセントは次に表示されているものよりも小さく、フォントとリソースのパーセントは大きくなるでしょう</p>
        </xsl:if>
        ...
        <p>アプリケーションには<code>include</code>文を使って参照しているファイルも含まれます。これらのファイル中の要素サイズは、以下のテーブルに含まれています。</p>
        
        <h2 id="classes">クラス定義</h2>

        <p>これはコンパイルされたクラス定義のサイズです。このサイズは未圧縮のサイズで、圧縮済アプリケーションより大きいサイズを示します。</p>
        <table border="1">
          <tr>
            <th>パス名:行番号</th>
            <th>名前</th>
            <th>サイズ<sup><a href="#size-legend">*</a></sup></th>
            <xsl:if test="$showpercent">
              <th>パーセンテージ<sup><a href="#percentage-legend">*</a></sup></th>
            </xsl:if>
          </tr>
          ...
          <tr>
            <td><i>合計</i></td>
            <td><i><xsl:value-of select="count(stats/block[@tagname='class'])"/> アイテム</i></td>
        ...
        <ul>
          ...
          <xsl:if test="$showpercent">
            <li id="percentage-legend">未圧縮アプリケーションサイズでのパーセンテージ。</li>
          </xsl:if>
        </ul>

        <h2 id="instances">トップレベルインスタンス</h2>

        <p>次はコンパイルされたトップレベルインスタンス(<code>canvas</code>と<code>library</code>)のサイズを表します。このサイズは未圧縮のサイズで、圧縮済アプリケーションより大きいサイズを示します。</p>
        
        <p>このサイズにはトップレベルにはすべての子要素が含まれています。</p>
        ...
        <p>これはインポートされた"だいたいの"リソースサイズを表します。いくつかのリソースは元のフォーマット(JPEGなど)からSWFへ変換されています。変換されたリソースのサイズは実際に出力されたプログラム内のものより小さく表示されています。</p>
        <table border="1">
          <tr>
            <th>名前</th>
            <?ignore <th>mimeタイプ</th> ?>
            <th>ソース</th> 
            <th>サイズ</th>
            <xsl:if test="$showpercent">
              <th>パーセンテージ</th>
        ...
        <h2 id="fonts">フォント</h2>
        <p>これはインポートされた"だいだい"のフォントサイズを表します。これらは元のフォーマット(TTF等)からSWFフォントフォーマットへ変換されています。 ここにはLFCに使われた追加のフォント・メートリックテーブルは含まれていませんが、特に小さいため気にする必要はありません。またSWFファイルを通してインポートされているフォントも含まれていません。</p>
        <table border="1">
          <tr>
            <th>フォント</th>
            <th>スタイル</th>
    ...
    <xsl:choose>
      <xsl:when test="$runtime = 'swf5'">       
        <div class="info">
          <table>
            <tr>
              <th align="left">実行環境ターゲット:</th><td><xsl:value-of select="$runtime"/></td>
            </tr>
            <tr>
              <th align="left">未圧縮:</th>
              <!-- print "ddK" or "d.dMB" -->
              <td>
                <xsl:choose>
      ...
      <xsl:otherwise>
        <div class="info">
          <table>
            ...
            <tr>
              <th align="left">アプリケーションサイズ (圧縮済):</th>
              <!-- print "ddK" or "d.dMB" -->
              <td>
                <xsl:choose>
                  <xsl:when test="$mb10 >= 9">
