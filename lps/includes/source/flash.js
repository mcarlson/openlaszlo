...
dojo.flash.Info.prototype = {
    ...
    _detectVersion: function(){
        ...
        //alert('_detectVersion ' + this.version);
    },
...
dojo.flash.Embed.prototype = {
    ...
    __getCSSValue: function(val){
        if (val && val.length && val.indexOf('%') != -1) {
            // scale container to 100% to allow the Flash player to scale appropraitely
            return '100%';
        } else {
            return val + 'px';
        }
    ...
    write: function(/* String */ flashVer, /* Boolean? */ doExpressInstall){
        ...
        if(flashVer == 6){
            ...
            if(doExpressInstall){
                ...
                swflocObjectVars += "&MMredirectURL=" + redirectURL
                                + "&MMplayerType=ActiveX"
                                + "&MMdoctitle=" + docTitle;
                swflocEmbedVars += "&MMredirectURL=" + redirectURL 
                                + "&MMplayerType=PlugIn";
            }
        ...
        if (div) {
            ...
            objectHTML = '<div id="' + divid + '" style="' + containerStyle + '"> '
                            + objectHTML
                        + '</div>';
            //alert('objectHTML' + objectHTML);
...
dojo.flash.Communicator.prototype = {
    _writeFlash6: function(){
        ...
        //alert("Writing flash 6 comm out");
    },
    ...
    if(command != null && RegExp("^FSCommand:(.*)").test(command) == true){
      command = command.match(RegExp("^FSCommand:(.*)"))[1];
        }
    ...
    _call: function(functionName, args){
        ...
        for(var i = 0; i < args.length; i++){
            ...
            var value = args[i];
            value = value.replace(RegExp("\\0", "g"), "\\0");
            
            plugin.SetVariable("_" + i, value);
        }
        
        // now tell Flash to execute this method using the Flash Runner
        //plugin.TCallLabel("flashRunner", "execute");
        //alert('_call ' + functionName + ', ' + args + ', ' + plugin);
        //alert('frame', plugin.TCurrentFrame("/flashRunner") );
        plugin.TCallFrame("/flashRunner", 1);
        //alert('frame', plugin.TCurrentFrame("/flashRunner") );
    ...
    var entityRE = RegExp("\\&([^;]*)\\;", "g");
        ...
        data = data.replace(RegExp("\\f", "g"), "\\f");
        data = data.replace(RegExp("\\0", "g"), "\\0"); // null character
        data = data.replace(RegExp("\\\'", "g"), "\\\'");
        data = data.replace(RegExp('\\\"', "g"), '\\\"');
    ...
    _decodeData: function(data){
        ...
        data = data.replace(RegExp("\\&custom_lt\\;", "g"), "<");
        data = data.replace(RegExp("\\&custom_gt\\;", "g"), ">");
