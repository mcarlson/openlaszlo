...
class SimpleCompiler(BaseEvaluator, CompilationContext):
    ...
    def import_stmt(self, names):
        ...
        for dotted, asname in names:
            ...
            # ows
            if dotted[0] in ('java', 'com', 'org') and asname:
                class ClassLookup(jast.Expression):
                    def __init__(self, value):
                        self.value = value
                    def sourceString(self):
                        return 'org.python.core.PyJavaClass.lookup(java.lang.Class.forName(%s))' % self.value

                class ChainException(jast.Expression):
                    def __init__(self, base, exception):
                        self.base = base
                        self.exception = exception
                    def sourceString(self):
                        return 'try {%s;} catch (%s e) {throw new RuntimeException(e.getMessage());}' % (self.base, self.exception)
                code = ClassLookup(modnameConst)
                code = self.get_module(dotted,1).makeReference(code)
                code = self.set_name(asname, code)
                code = ChainException(code, 'ClassNotFoundException')
                #print code
                ret.append(code)
                continue
            if asname:
